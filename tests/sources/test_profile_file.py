...
from ``env`` (read from an injectable environment) or ``literal`` (copied
verbatim) into a :class:`MappingProfileResolver`.  Every malformed document or
unresolvable value surfaces as a :class:`ProfileFileValidationError` carrying
only a stable ``code``, a structural ``path``, and a constant message — never a
resolved value, an environment value, or an environment name.
"""
...
def test_profile_file_passes_through_every_literal_type(tmp_path: Path) -> None:
    path = _write(
        tmp_path,
        "version: 1\n"
        "profiles:\n"
    ...
    profile = resolver.resolve("warehouse", source_id="orders")
    assert profile.value("a_str") == "hello"
    assert profile.value("an_int") == 42
    assert profile.value("a_float") == 3.14
    assert profile.value("a_null") is None
    assert profile.value("a_list") == ["a", "b"]
    assert profile.value("a_map") == {"nested": 1}
    assert profile.value("a_bool") is True
...
def test_profile_file_supports_multiple_profiles(tmp_path: Path) -> None:
    ...
    assert resolver.resolve("cache", source_id="orders").value("url") == "memory://"
    assert (
...
def test_profile_file_resolver_returns_runtime_profile(tmp_path: Path) -> None:
    ...
    assert profile.value("region") == "eu"
