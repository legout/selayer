...
def _plan_outcome(
    ...
    is ``passed`` with ``compatible`` set to ``False``. Only the planner's
    safe, identifier-only messages are recorded.
    """
    try:
...
def verify_compatibility(
    ...
    request set, each planned once. Explicit ``query_cases`` are passed to the
    planner verbatim, so an unknown selector inside a query case surfaces as a
    planner incompatibility (``compatible=False``), not a declaration failure.
    """
    requested_metrics = (
    ...
    requests = _build_requests(
        tuple(valid_metrics), tuple(valid_dimensions), check.query_cases
    )
