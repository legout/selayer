...
def create_ecommerce_semantic_layer():
    ...
    # Add data sources
    sl.add_data_source(
        DataSource(name="orders", type="parquet", path="data/orders.parquet")
    )

    sl.add_data_source(
        DataSource(name="order_items", type="parquet", path="data/order_items.parquet")
    )

    sl.add_data_source(
        DataSource(name="customers", type="parquet", path="data/customers.parquet")
    )

    sl.add_data_source(
        DataSource(name="products", type="parquet", path="data/products.parquet")
    )

    sl.add_data_source(
        DataSource(name="campaigns", type="parquet", path="data/campaigns.parquet")
    )
    ...
    sl.add_data_source(
        DataSource(
            name="website_visits", type="parquet", path="data/website_visits.parquet"
        )
    )
...
def test_semantic_layer():
    ...
    Path("ecommerce_semantic_layer.mermaid").write_text(
        mermaid_diagram, encoding="utf-8"
    )

    print("Running example metric queries...")
    with QueryEngine(loaded_sl) as engine:
