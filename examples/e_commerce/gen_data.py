...
def generate_customers(num_customers=1000):
    ...
    for i in range(num_customers):
        ...
        customers.append({
            ...
            "lifetime_value": round(random.uniform(0, 2000), 2)  # Initial LTV value
        })
    ...
    for i, customer_id in enumerate(customer_ids):
        ...
        for _ in range(num_customer_orders):
            ...
            order_date = fake.date_time_between(
                start_date=customers_df.loc[customers_df["id"] == customer_id, "registration_date"].iloc[0],
                end_date="now"
            )
            ...
            if status in ["returned", "cancelled"]:
                reasons = ["Changed mind", "Found better price", "Damaged", "Wrong item", "Delayed shipping"]
            ...
            orders.append({
                ...
                "reason": reason,
                "is_first_purchase": order_id_counter <= len(customer_ids)  # First purchase for each customer
            })
            
            # Generate 1-5 items per order
            num_items = random.randint(1, 5)
            order_products = np.random.choice(product_ids, size=num_items, replace=False)
            ...
            orders[-1]["total_amount"] = round(order_total + shipping_cost - discount_amount, 2)
...
def generate_marketing_touches(customers_df, campaigns_df, num_touches=10000):
    ...
    campaign_dates = dict(zip(campaigns_df["id"], zip(campaigns_df["start_date"], campaigns_df["end_date"])))
    ...
    for i in range(num_touches):
        ...
        touches.append({
            ...
            "touch_date": fake.date_time_between(start_date=start_date, end_date=end_date),
            "channel": np.random.choice(["Email", "Web", "Mobile", "Social"]),
            "interaction": np.random.choice(["View", "Click", "Conversion", "None"], p=[0.6, 0.3, 0.05, 0.05])
        })
...
def generate_website_visits(customers_df, num_visits=20000):
    ...
    pages = [
        ...
        "/product/detail", "/cart", "/checkout", "/account", "/support"
    ]
    ...
    for i in range(num_visits):
        ...
        for j in range(num_pages):
            ...
            time_on_page = round(random.expovariate(1/60), 1)  # Average 60 seconds per page
            ...
            visits.append({
                ...
                "referrer": np.random.choice(["Google", "Facebook", "Direct", "Email", "Other"], p=[0.4, 0.2, 0.2, 0.1, 0.1])
            })
...
def generate_customer_support_tickets(customers_df, orders_df, num_tickets=2000):
    ...
    for i in range(num_tickets):
        ...
        tickets.append({
            ...
            "response_time_hours": random.randint(1, 72) if status != "Open" else None,
    ...
    while current_date <= end_date:
        for product_id in product_ids:
            ...
            if current_date == start_date:
                ...
                prev_stock = next((s["stock"] for s in snapshots if s["product_id"] == product_id and s["date"] == current_date - timedelta(days=1)), initial_stock[product_id])
                
                # Simulate sales and restocks
                sales = max(0, int(np.random.poisson(0.5)))  # Average 0.5 units sold per day
                restock = int(np.random.poisson(0.2) * 10)   # Occasional restocks of ~10 units
...
def main():
    ...
    marketing_touches = generate_marketing_touches(customers, campaigns, num_touches=10000)
    
    print("Generating website visits...")
    website_visits = generate_website_visits(customers, num_visits=20000)
    
    print("Generating support tickets...")
    support_tickets = generate_customer_support_tickets(customers, orders, num_tickets=2000)
    ...
    print(f"Generated {len(campaigns)} marketing campaigns with {len(marketing_touches)} customer touches")
